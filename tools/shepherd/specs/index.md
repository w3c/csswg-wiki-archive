...
1. A 'data-dfn-type' attribute on the <dfn> with a valid type <code><dfn id=“image-element” data-dfn-type=“element”>image</dfn></code>
1. An 'id' on the <dfn> (or container <a> or <dt> with the 'id') that has one of the following prefixes: <code><dfn id=“elementdef-image”>image</dfn></code>
  - 'propdef-' -> 'property'
  - 'valuedef-' -> 'value'
  - 'valdef-' -> 'value'
  - 'vdef-' -> 'value'
  ...
  - 'schemedef-' -> 'scheme'
1. If the <dfn>'s 'id' has one of the following “magic” prefixes (for legacy support, not recommended for use in new specs):
  ...
  - 'dom-' -> some IDL construct - WebIDL found in the specification will be searched for a construct having the same name as the text contents of the <dfn>, if not found, then use the heuristics:
    ...
    1. if it looks like a function `(match '^[^ ]+\([^\(]*\)$')` -> 'method'
    1. otherwise -> 'attribute'
1. Search WebIDL found in the specification for a construct having the same name as the text, if found, use that type
1. Look for the nearest ancestor with a valid 'data-dfn-type' attribute, or one of the following classes:
  - 'propdef' -> 'property'
  - 'valuedef' -> 'value'
  - 'valdef' -> 'value'
  - 'vdef' -> 'value'
  ...
  - 'http-headerdef' -> 'http-header'
  - 'schemedef' -> 'scheme'
1. Look at the text content of the <dfn> and use the following heuristics:
  1. If the <dfn> content starts with '@' -> 'at-rule' <code><dfn>@foo</dfn></code>
  1. If the <dfn> content is quoted, e.g.: `"foo", 'foo', “foo”, or 'foo'` -> 'value' <code><dfn>"foo"</dfn></code>
  1. If the <dfn> content starts with '<' and ends with '>' -> 'type' <code><dfn>&lt;foo&gt;</dfn></code>
  1. If the <dfn> content starts with ':' -> 'selector' <code><dfn>:foo</dfn></code>
  1. If the <dfn> content looks like a function and the 'id' does not start with 'dom-' -> 'function' <code><dfn>foo(bar)</dfn></code>
...
1. For <p>, <dt>, and section heading elements that are anchors, if the element contains a <span> that is not an anchor and has a 'class' attribute
  ...
  1. if one of the classes is 'adef' -> 'element-attr'
  1. if one of the classes is 'SVG-TermDefine' -> 'concept'
  1. if the element is a section heading and one of the classes is 'prop-name' -> 'property'
  1. if the element is a section heading and one of the classes is 'element-name' -> 'element',
1. For section headings, if the anchor name and the section name both start with 'Interface' -> 'interface' 
...
1. Use the value of the <dfn>'s 'data-dfn-for' attribute <code><dfn id=“image-width” data-dfn-type=“element-attr” data-dfn-for=“image”>width</dfn></code>
1. If the <dfn> is an IDL construct, determine the relationship from the IDL
1. Look for the nearest element ancestor with a 'data-dfn-for' attribute <code><div data-dfn-for=“image” data-dfn-type=“element-attr”>
