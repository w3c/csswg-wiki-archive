---
title: "Quick Guide To Using Git"
---

> [!WARNING]
> This page is a work in progress, copy-pasted from the Mercurial guide, to be turned in to a git guide. It is not fully updated yet, so it may not make sense.
>
> # Quick Guide To Using Git
>
> The CSS Working Group has adopted Git as the repository technology for storing our test suites and editor's drafts of specifications. Git is a modern, distributed version control system. This guide is intended to serve as a quick start for those setting up access to the repository for the first time, not to serve as an in-depth tutorial. It is expected that the reader have some familiarity with centrally managed version control systems like CVS or Subversion.
>
> This guide will focus on the command-line git client `git`, but there are GUI-based git clients available for many platforms. While users of a GUI client obviously won't be using the command line, the same basic operations apply.
>
...
> ### Installing Git
>
> You need to first [install git on your system](../../tools/git/install/ "tools:git:install"), if you haven't already. We recommend using the latest client.
>
...
>
> >
> This section and the ones below have not yet been updated, and still talk about mercurial. If you update it, please move this warning down after doing so. If you were looking for instructions, come back some other time, they're not here yet.
>
> Note: [Detailed instructions for the config file](http://www.selenic.com/mercurial/hgrc.5.html) are available, but for now, this guide will focus only on setting up the basics.
>
...
>
> Putting your password into `.hgrc` will prevent the server from asking your username and password every time you push. If you don't want to store your password in the config file, you can [install the Keyring extension](http://mercurial.selenic.com/wiki/KeyringExtension) and omit the password line. The Keyring extension will prompt you for your password the first time it is needed and store it securely in your system keyring.
>
> If you access the web through a proxy, you'll also need to set that up now. [Details of proxy settings can be found here](http://www.selenic.com/mercurial/hgrc.5.html#http-proxy).
>
> You can optionally [select an interactive merge program](http://mercurial.selenic.com/wiki/MergeProgram) in place of `internal:merge`. The merge program will help you resolve conflicts between your work and others' after pulling from the central repository. With `internal:merge`, Mercurial simply leaves conflict markers in your source, similar to Subversion or other systems.
>
> >
> **Avoiding Fingerprint Warnings (and improving security)**
>
> Depending on your OS and Mercurial root certificate configuration you may get a warning message about the host certificate when communicating with the central repository. If that happens, follow [the instructions on the Mercurial wiki](https://www.mercurial-scm.org//wiki/CACertificates#Configuration_of_HTTPS_certificate_authorities).
...
Access to the repositories on hg.csswg.org, hg.fxtf.org, and hg.css-houdini.org is based on your account for [Shepherd](http://test.csswg.org/shepherd/) and this wiki (user accounts are shared between systems). By default, all CSSWG members should have write access to the Test Suite Repository. Other contributors to the Test Suite Repository need to ask for access. If you already have an account on this wiki, log in to Shepherd and submit a request to modify assets on the [Repository Access](https://test.csswg.org/shepherd/login/account/access/) page. You will receive an email once repository access has been granted. If you don't already have an account, you can [register for an account on Shepherd](https://test.csswg.org/shepherd/register/).

In addition to the Test Suite Repository, all CSSWG members should have write access to the CSSWG Draft Repository, all CSSWG and SVGWG members should have write access to the FXTF and CSS-Houdini TF Draft Repositories. If you need write access and don't already have it, you can submit a request on the [CSSWG Repository Access](https://drafts.csswg.org/login/account/access/), [FXTF Repository Access](https://drafts.fxtf.org/login/account/access/), or [CSS-Houdini TF Repository Access](https://drafts.css-houdini.org/login/account/access/) pages.

Access to the repositories on dvcs.w3.org is controlled by your W3C user account. If you're not a member of the appropriate group and need write access, please email [Peter](mailto:mailto:peter.linss@hp.com) or [sysreq](mailto:mailto::sysreq@w3.org).

## Working With Mercurial

Once you have a created a working directory by cloning the central repository onto your machine, you are free to edit the files in the working directory with your favorite tools. Mercurial will automatically detect edits made to any files within your working directory. However, when moving, renaming, copying, or deleting files, these operations need to be done [via Mercurial](#manging-files "tools:git ↵") rather than your standard operating system operations. This enables Mercurial to track those changes in addition to edits within the files.
...
1.  Synchronize my local repository and its working directory with the central repository
2.  Make edits in my working directory
3.  Commit my work into my local repository
4.  Send my changes back to the central repository
...
- [Examining Your Changes](#examining-your-changes "tools:git ↵")
  - [Listing Changed Files: hg status](#listing-changed-fileshg-status "tools:git ↵")
  - [Diffs and Patches: hg diff](#diffs-and-patcheshg-diff "tools:git ↵")
  - [Reverting Changes: hg revert](#reverting-changeshg-revert "tools:git ↵")
- [Synchronizing Changes](#synchronizing-changes "tools:git ↵")
  - [Updating Your Repository: hg pull](#updating-your-repositoryhg-pull "tools:git ↵")
  - [Submitting Your Changes: hg commit and hg push](#submitting-your-changeshg-commit-and-hg-push "tools:git ↵")
  - [Merging Branches of Work: hg merge](#merging-branches-of-workhg-merge "tools:git ↵")
  - [Resolving Conflicts: hg resolve](#resolving-conflictshg-resolve "tools:git ↵")
- [Managing Files](#managing-files "tools:git ↵")
  - [Adding New Files: hg add](#adding-new-fileshg-add "tools:git ↵")
  - [Removing Files: hg remove](#removing-fileshg-remove "tools:git ↵")
  - [Moving and Renaming Files: hg move](#moving-and-renaming-fileshg-move "tools:git ↵")
  - [Copies and Forks: hg copy](#copies-and-forkshg-copy "tools:git ↵")
- [Working Offline](#working-offline "tools:git ↵")
  - [Pull vs. Update](#pull-vs-update "tools:git ↵")
  - [Commit vs. Push](#commit-vs-push "tools:git ↵")
  - [Patch Queues](#patch-queues "tools:git ↵")
  - [Rebasing Changesets](#rebasing-changesets "tools:git ↵")
  - [Comparing Repositories](#comparing-repositories "tools:git ↵")
- [Archaeology](#archaeology-and-travelling-through-time "tools:git ↵")

### Examining Your Changes

This section covers:

- [Listing Changed Files](#listing-changed-fileshg-status "tools:git ↵")
- [Diffs and Patches](#diffs-and-patcheshg-diff "tools:git ↵")
- [Reverting Changes](#reverting-changeshg-revert "tools:git ↵")
...
See [Managing Files](#managing-files "tools:git ↵") for adding, removing, and moving files with Mercurial.
...
- [Updating Your Repository](#updating-your-repositoryhg-pull "tools:git ↵")
- [Submitting Your Changes](#submitting-your-changeshg-commit-and-hg-push "tools:git ↵")
- [Merging Branches of Work](#merging-branches-of-workhg-merge "tools:git ↵")
- [Resolving Conflicts](#resolving-conflictshg-resolve "tools:git ↵")
...
❓ If you have locally-committed changes that you haven't yet pushed to the server, you might see a message about multiple “heads”. In this case you need to `hg merge`. See [Merging](#merging-branches-of-workhg-merge "tools:git ↵"), below.
...
> [!NOTE]
> Before you commit your changes, it's a good idea to review the changes with `hg status`, making sure new files have been `hg add`ed, deleted files have been `hg remove`d; and double check an `hg diff` of your edits. It is also good practice to commit your work often and in small increments, grouping only related changes together.
>
...
- [Adding New Files](#adding-new-fileshg-add "tools:git ↵")
- [Removing Files](#removing-fileshg-remove "tools:git ↵")
- [Moving and Renaming Files](#moving-and-renaming-fileshg-move "tools:git ↵")
- [Copies and Forks](#copies-and-forkshg-copy "tools:git ↵")
...
Another way to manage offline changes is to use [Mercurial Queues](http://mercurial.selenic.com/wiki/MqTutorial), which manages your local changes as pseudo-commits that you can apply, unapply, revise, and rearrange before you formally commit and push. Details about using patch queues are beyond the scope of this document.
...
⚠️ If you are working on a decentralized project, don't rebase any changesets that others have pulled from you. (Rebasing changes a changeset's ID, causes all kinds of confusion if those changesets are shared.)
...
If you're stuck, you can [email Peter](mailto:mailto:peter.linss@hp.com) for help with the repository.
