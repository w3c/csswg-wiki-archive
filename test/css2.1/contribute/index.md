...
- All tests must also adhere to the [format guidelines](/test/format/).
...
To submit a test, simply add it to the `submitted` folder in your contributor directory and push your changes to `https://hg.csswg.org/test`. It will show up in [Shepherd](http://test.csswg.org/shepherd/), our test-tracking system. You can use the `incoming` folder in your contributor directory as scratch space. See [How to use Mercurial](/tools/hg/).

Please make sure submissions follow the [CSS2.1 Test Suite Guidelines](http://www.w3.org/Style/CSS/Test/guidelines.html) and are in the proper [format](/test/format/). New tests are subject to the [review process](/test/review/). We may ask you to fix problems in your test and resubmit it before adding it to the test suite.
...
Fixes to existing tests are also subject to the [review process](/test/review/).
...
- You can convert self-describing tests into [reftests](/test/reftest/) by creating references for htem.
