...
1. rules that change the context of rules inside them (like @media or @region)
1. rules that apply properties to things in the document other than elements (like @page or @viewport)
1. rules that define values which are too complex to define inline (like @font-face or @keyframes)
...
Category 3 rules typically cascade descriptors individually, like properties.  If they have some notion of "selectors" with specificity (like [@page rules](http://dev.w3.org/csswg/css3-page/#cascading-and-page-context)), define it.  Also, define whether !important is accepted - accept it if there are multiple objects and you have some notion of "selectors" like @page does, but don't accept it (make it a syntax error) if there is only a single object like @viewport.
...
**A new constant** (coordinated at the [CSSOM Constants](/CSSOM Constants/) page), added to the CSSRule interface like:
