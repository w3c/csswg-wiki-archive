...
When writing the tests for a specification, the test must contain one or more [links](/test/format#links/) to the area(s) of the specification being tested. The fragment identifier of the link should be the 'id' of the heading or containing element of the assertion(s) under test.

## Creating a test suite

Test suites are currently created manually. This will soon be a fully automated process.

Once a sufficient number of tests have been submitted, [email Peter](/mailto/peter.linss@hp.com/) to have the test suite generated. At that point, the test suite will be built daily and available for testing in the [test harness](http://test.csswg.org/harness).
