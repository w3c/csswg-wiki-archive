...
1. JIS/JLREQ/InDesign style (trim and specify how much to add), or specify how much to add/remove?
  1. Results differ if punctuation are proportional
1. Spaces are not consistent for: "]<big>[]</big>["
1. Can 'none' have optional min/max/alternate/priority? Does it allow expansion?
1. How to handle proportional punctuation is still an issue
1. '/' syntax is used only for closing+line-end, where CSS3 Text defines by the "if it fits" clause
