...
Yes, change to exclude overflow:hidden was intentional. The understanding in the WG is that all block formatting context roots inhibit margin collapsing through their border edges, and there is interop on this behavior. <http://software.hixie.ch/utilities/js/live-dom-viewer/?%3C!DOCTYPE%20html%3E%0D%0A%3Cdiv%20style%3D%22border%3A%20solid%22%3E%0D%0A%20%3Cdiv%20style%3D%22margin%3A%201em%3Boverflow%3A%20hidden%22%3E%3C%2Fdiv%3E%0D%0A%3C%2Fdiv%3E%0D%0A>
...
12.5 has been already edited to specify “principal block box”, which is more correct.
...
Will add “e.g.” to indicate that the list (of one 'list-item' ;) is not exhaustive. See Issue 271.
...
There doesn't seem to be an actual problem here, so not fixing. (There is some new wording here that should make it clearer, in any case.)
...
The rules for generating anonymous boxes specify that the block-level box causing their generation must be in-flow, so the float doesn't cause a problem here.
