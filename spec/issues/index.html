...
Your primary weapon in the issue-whacking arts is the trusty Issue Tracker. There are many to choose from:
  ...
  * [[https://www.w3.org/Bugs/Public/|Bugzilla]]
  * Tracker ([[https://www.w3.org/Style/CSS/Tracker/|CSSWG]]/[[https://www.w3.org/Graphics/fx/track/|FXTF]])
  * This wiki's [[http://wiki.csswg.org/spec|spec pages]]
...
Note that in the CSSWG **all issue-resolving discussion happens on the mailing list** (or at a telecon/F2F whose minutes are posted to the mailing list). Which means your issue-tracker is primarily an index into [[http://lists.w3.org/Archives/Public/www-style/|www-style]]/[[http://lists.w3.org/Archives/Public/public-fx/|public-fx]], not a place to discuss the issue.
...
Currently, the only CSSWG issue-tracker that offers this feature is plaintext. See [[http://dev.w3.org/csswg/bin/issuegen.pl|IssueGen]].
...
Part of this responsibility is the technical and editorial task of solving the issue consistent with CSS’s design principles and making edits accordingly.
  * [[http://www.w3.org/TR/CSS21/intro.html#design-principles|CSS Design Principles]], as recorded in the CSS2.1 specification.
  * [[http://fantasai.inkedblade.net/weblog/2012/css-layout-evolution/#css3-overview|CSS Layout: Principles of Design]], as presented by fantasai
  * [[http://wiki.csswg.org/spec#coordination-between-specifications|CSS Cross-module Coordination]], tips for cross-spec consistency
...
As a [[http://fantasai.inkedblade.net/weblog/2011/inside-csswg/decisions|delegate of the CSSWG]], you need to decide when it's appropriate to resolve an issue by yourself, and when it's appropriate to raise it to the CSSWG. Here are some guidelines:
...
You can look at the CSSWG as a bureacratic approval committee that you must appease. Or, you can look at it as a group of really smart and knowledgeable people who care about making your spec better but whose attention you need to cultivate because we're all way too overloaded to read every thread on www-style. Personally, I start with the assumption that the CSSWG’s review would be very useful on everything, but attention is a limited resource so my job is to
  - Filter out issues they don't need to care about because they're trivial.
  - Do a really good job of summarizing the issue so as many people as possible can really understand the problem and the proposed solutions (if any) and help resolve the issue better than I could by myself.
  - Communicate any major changes and additions at a high level so everyone is informed of what's going on, can adjust implementation plans as needed, allocate time for a detailed review if appropriate, coordinate their module’s design and development with mine, and give feedback on my work that incorporates experience, expertise, and implementation constraints that I'm not aware of.
...
As a CSSWG editor, you are expected to periodically update the "TR copy", that is, the dated snapshot listed on http://www.w3.org/TR/. The [[spec:publish|CSS Spec Publishing Process]] is a somewhat tedius process which has three main phases:
  - Announcing your intent to publish to the WG.
  - Fussing with the W3C publication process
  - Announcing your publication as fait accompli.
