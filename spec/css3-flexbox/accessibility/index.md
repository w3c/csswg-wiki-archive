...
1. Flexbox will lose the intended opportunity to rearrange the visual order without changing the logical tabbing order for keyboard users
1. It will be difficult to implement and track the logical order change to the box tree and the accessibility tree
1. Flexbox is strongly hierarchal thus limiting the damage that can be done
1. Visual scanning order is not always the same as the logical order so it’s not possible to insure a common experience
1. Devs should not be using Flexbox to fix problems with the layouts
1. Blind users will end up with a different experience on mobile than desktop
...
1. No evidence or example that this is a desirable outcome
1. This is the direction CSS is moving and the risk is too great to ignore due to effort
1. We have examples of Flexbox creating terrible, non-compliant visual layouts, especially in complex applications
1. The conventional order of tabbing through a document is still desired rather than jumping around
1. Devs will continue to do this regardless of best practices. It’s easy and has major performance implications
1. Blind users want the same experience that visual users get in each context
...
1. High Contrast mode users are at risk of jumping around a screen causing confusion as they see only a small portion of the screen
1. Visual Keyboard users have difficulty tabbing to information that visually shows up early but is logically near the end. Example: navigation list that is logically at the end but moved to the middle left using flexbox
1. Description of location for blind users will be impossible if the location of the information does not match visually. Example: Blind user calling a help line to describe an issue in a portion of a page. Example: Blind user having a conversation with coworkers about a screen layout
1. PF Group is already concerned with this issue http://lists.w3.org/Archives/Public/www-style/2014May/0247.html
1. IBM has an extremely strong stance on this
