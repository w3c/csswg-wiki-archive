...
The issue of using region divs in HTML markup has been around since the initial presentation to the CSS WG ([early feedback](http://lists.w3.org/Archives/Public/www-style/2011Mar/0189.html)). Everyone seems to agree that there should be additional mechanisms to create stylable boxes in the render tree from CSS itself ([pseudo-elements](http://wiki.csswg.org/ideas/pseudo-elements)). Regions is not the only specification that could benefit from this addition to CSS.
...
1. Is a new box-generation mechanism required for css3-regions to progress? If so, does it belong
  ...
  1. a separate specification that css3-regions must reference?
