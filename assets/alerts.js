/**
 * Convert GFM-style alerts to styled HTML.
 * Looks for blockquotes starting with [!NOTE], [!TIP], [!IMPORTANT], [!WARNING], [!CAUTION]
 */
(function() {
  const ALERT_TYPES = {
    NOTE: {
  ...
  document.querySelectorAll('blockquote').forEach(function(bq) {
    ...
    // Get text content and check for alert marker
    const html = firstP.innerHTML;
    const match = html.match(/^\[!(NOTE|TIP|IMPORTANT|WARNING|CAUTION)\][\s\n]*/);
    if (!match) return;
    ...
    // Add alert class and create title
