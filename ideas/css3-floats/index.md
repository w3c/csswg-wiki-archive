...
THIS PAGE IS DEPRECATED AND REPLACED BY THE [CSS exclusions and shapes](/ideas/css3-exclusions/) page.
...
There is a companion [use cases](/ideas/css3-floats-use-cases/) page.
...
1. A BFC is set up and shrink-wrapped
1. The BFC is positioned
    1. Relative to the marker left behind when the content is removed
    1. Floats do not overlap one another and are stacked or cleared
    1. Positioning is limited to the “containing block”
1. The BFC is an exclusion using it Border box as the wrap-shape
 
For exclusions,
1. Positioning removes the content from normal flow (or the content is left in normal flow)
1. Positioning also sets up a BFC and shrink-wraps it; wrap-shape may be default or explicit
1. Positioning positions the exclusion,
    1. The marker is used to resolve “auto” values in absolute positioning
    1. The Exclusions proposal has groups and order to control exclusion interaction
    1. Interaction is not limited per b.
1. The wrap-shape (or pair of shapes) is the exclusion.
...
The use of 'wrap' in properties such as 'wrap-type' or 'wrap-mode' may be confusing because of a different meaning for wrap in the FlexBox specification ('flex-wrap' property) see Flex Box [issues](/spec/css3-flexbox/#issue-1/).
